...
class LoginForm extends Component {
    handleSubmit = e => {
        ...
        e.preventDefault();
        message.loading({ content: '登录中...', duration: 0.4 });
        const { validateFields } = this.props.form
        validateFields((err, values) => {
            if (!err) {
                values.password = md5(values.password + values.username);
                axios.post('/login', values).then((res) => {
                    const { data, token, code, msg } = res.data;
                    if (code === 200) {
                        const { Role } = data;
                        ...
                        this.props.history.push('./' + Role.roleName)

                    } else if (code == 400) {
                        message.warn({ content: msg });
                    } else {
                        message.warn({ content: '其他错误' });
                    }
                }).catch((err) => {
                    console.log(err)
                    message.warn({ content: '服务器错误' });
                })
            }
        });
    }
